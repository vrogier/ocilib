...
inline ostring GetArg(oarg *arg)
{
    ...
    #define ORACLE_BUFFER_SIZE 1000
    ...
    void mainsssss()
    {
        std::vector<std::tuple<int, std::string>> container;

        for (auto i = 1; i < INPUT_SIZE * 2; i++)
        {
            container.push_back({ i, "value " + std::to_string(i) });
        }

        OCI_Initialize(err_handler, nullptr, OCI_ENV_DEFAULT);

        auto cn = OCI_ConnectionCreate("db18c", "usr", "pwd", OCI_SESSION_DEFAULT);
        auto st = OCI_StatementCreate(cn);

        OCI_Prepare(st, "INSERT INTO mytable (id, str) VALUES(:1, :2)");
        OCI_BindArraySetSize(st, ORACLE_BUFFER_SIZE); 

        auto values = std::unique_ptr<int[]>(new int[ORACLE_BUFFER_SIZE]);
        OCI_BindArrayOfInts(st, ":1", values.get(), 0);

        auto size_without_null_terminal_character = 1000;
        auto max_size_with_null_terminator = 1000 + 1;

        auto str_values = std::unique_ptr<char[]>(new char[ORACLE_BUFFER_SIZE * max_size_with_null_terminator]);
        OCI_BindArrayOfStrings(st, ":2", str_values.get(), size_without_null_terminal_character, 0);

        for (auto container_index = 0; container_index < container.size(); container_index += ORACLE_BUFFER_SIZE)
        {
            auto partial_index = 0;
            for (; (partial_index < ORACLE_BUFFER_SIZE) && (container_index + partial_index < container.size()); ++partial_index)
            {
                auto [val, str_val] = container[container_index + partial_index];

                values[partial_index] = val;
                std::copy(begin(str_val), end(str_val), &str_values[partial_index * max_size_with_null_terminator]);
            
                str_values[partial_index * max_size_with_null_terminator + str_val.size()] = '\0';
            }

            OCI_BindArraySetSize(st, partial_index);
            OCI_Execute(st);
        }
...
void mainsss()
{
    ...
    {
        container.push_back({ i, std::to_string(i) });
    }

    try
    {
        Environment::Initialize(Environment::Default | Environment::Threaded);
        Connection con("db18c", "usr", "pwd", Environment::SessionDefault);
        Statement st(con);

        std::vector<int> ids;
        std::vector<std::string> values;

        ids.resize(ChunkSize);
        values.resize(ChunkSize);

        st.Prepare("INSERT INTO mytable (id, str) VALUES(:1, :2)");
        st.SetBindArraySize(ChunkSize);
        st.Bind(":1", ids, BindInfo::In);
        st.Bind(":2", values, 50, BindInfo::In);

        for (auto container_index = 0; container_index < container.size(); container_index += ChunkSize)
        {
            auto partial_index = 0;
            for (; (partial_index < ChunkSize) && (container_index + partial_index < container.size()); ++partial_index)
            {
                auto [val, str_val] = container[container_index + partial_index];

                ids[partial_index] = val;
                values[partial_index] = str_val;
            }

            st.SetBindArraySize(partial_index);
...
std::optional<short> state;
std::optional<std::string> serial_no;
...
int main()
{
    ...
    ocilib::Pool oci_pool("localhost:1521/db19c", "usr", "pwd",
        ocilib::Pool::ConnectionPool, 200, 300);



    auto worker_lambda = [](ocilib::Pool& pool)
    {
        ...
        try
        {
            ...
            auto entity_db_obj = ocilib::Object(ocilib::TypeInfo(conn, "MY_ENTITY_OBJ", ocilib::TypeInfo::Type));

            entity_db_obj.Set("ID", 2);
            
            std::cout << (void*) &entity_db_obj << std::endl;
            /*
            std::string sql = R"(
                            BEGIN
                                :RESULT := MY_PACKAGE_PKG.GET_ENTITY_BY_CODE(:CODE , :ENTITY_INFO);
                            END;
                            )";
            stmt.Prepare(sql);
            stmt.Bind(":RESULT", result, ocilib::BindInfo::Out);
            std::string entity_code = "00000001";
            stmt.Bind(":CODE", entity_code, 8, ocilib::BindInfo::In);
            stmt.Bind(":ENTITY_INFO", entity_db_obj, ocilib::BindInfo::Out);

            stmt.ExecutePrepared();

            if (result)
            {
                my_entity one_entity;
                if (!entity_db_obj.IsAttributeNull("ID"))
                    one_entity.id = entity_db_obj.Get<int>("ID");

                if (!entity_db_obj.IsAttributeNull("CODE"))
                    one_entity.code = entity_db_obj.Get<std::string>("CODE");

                if (!entity_db_obj.IsAttributeNull("STATE"))
                    one_entity.state = static_cast<short>(entity_db_obj.Get<short>("STATE"));

                if (!entity_db_obj.IsAttributeNull("SERIAL_NO"))
                    one_entity.serial_no = entity_db_obj.Get<std::string>("SERIAL_NO");
    ...
    for (int i = 0; i < 1; i++)
    {
        threads[i] = std::thread(
            worker_lambda, std::ref(oci_pool)
        );
    }
